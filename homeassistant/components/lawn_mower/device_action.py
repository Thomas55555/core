...
from . import DOMAIN, SERVICE_DOCK, SERVICE_PAUSE, SERVICE_START_MOWING
from .const import LawnMowerEntityFeature
...
async def async_get_actions(
    ...
    for entry in er.async_entries_for_device(registry, device_id):
        ...
        if supported_features and LawnMowerEntityFeature.START_MOWING:
            actions.append({**base_action, CONF_TYPE: "mow"})
        if supported_features and LawnMowerEntityFeature.DOCK:
            actions.append({**base_action, CONF_TYPE: "dock"})
        if supported_features and LawnMowerEntityFeature.PAUSE:
